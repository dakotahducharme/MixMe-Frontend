...
class App extends Component {
  ...
  getArtist = async (q) => {
    try {
      await fetch(`${process.env.REACT_APP_API}/api/spotify/artist?query=${q}`)
        .then(res => {
          res.json().then(data => {
  ...
  getRecommendations = async () => {
    try{
      await fetch(`${process.env.REACT_APP_API}/api/spotify/recommendations?artists=${this.seedArtists()}`)
      .then(res => {
        res.json().then(data => {
  ...
  createMix = async () => {
    ...
    try{
      await axios.post(`${process.env.REACT_APP_API}/api/mixes`, { artists, songs })
      .then(res => {
        console.log(res)
        this.getMixes();
      }
  ...
  getMixes = async() => {
    await fetch(`${process.env.REACT_APP_API}/api/mixes/`)
    .then( res => {
  ...
  deleteMix = async(id) => {
    await axios.delete(`${process.env.REACT_APP_API}/api/mixes/${id}`).then(() => {
      console.log('deleted')
      this.getMixes()
    }).catch(err => console.log(err))
  }
